...
export class NeuralNetwork implements NeuralNetworkArgs{
  ...
  public addNeuronToLayer(layerI: number) {
    // layerI being -1 means adding an input neuron
    if(layerI < -1 || layerI >= this.layers.length) throw new Error("addNeuronToLayer: invalid layer number: " + layerI);
    var layer = this.layers[layerI];
    var nextLayer = this.layers[layerI + 1];
    if(layer)     layer.addNeuron();
    if(nextLayer) nextLayer.addInput();

    if(layerI == -1) this.numInputs++;
    else if(layerI == this.layers.length - 1) this.numOutputs++;
    else this.neuronsPerHiddenLayer++;
  }
  ...
  public stochasticGradientDescent(trainingData: TrainingData, epochs: number,
    ...
    for(var i=0; i < epochs; ++i){
      ...
      //////////// TEST:
      if("TEST" == "TEST" && i % 3 == 0){
        this.addNeuronToLayer(0);
        console.log("Middle layer num neurons:", this.neuronsPerHiddenLayer);
      }
